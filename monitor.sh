...
# num is to check the number of files in reports_dir
num=0
...
function generate_report {
  ...
  if [ $num -eq $MAXIMUM_REPORTS ]; then
    report=$(ls -t $REPORTS_DIR | tail -1)
    rm -r "$REPORTS_DIR/$report"
    num=$((num - 1))
  fi
...
function notify
{
  ...
  if [[ "$cpu_usage_int" -gt "$CPU_THRESHOLD" || "$2" -gt "$MEMORY_THRESHOLD" ]];then
     lastreport=$(ls -t ./reports_dir | head -1)
     /usr/bin/mailx -s "monitor report" $USER < ./reports_dir/$lastreport
  fi
}
